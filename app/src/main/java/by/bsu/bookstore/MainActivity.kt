...
class MainActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        promotionsViewPager = findViewById(R.id.promotionsViewPager)
        sectionsRecycler = findViewById(R.id.sectionsRecyclerView)
    ...
    private fun setupPromotions() {
        val promotions = listOf(
            Promotion("Скидка 20% на классику", "Только до конца месяца!"),
            Promotion("Новинки со скидкой", "Лучшие книги 2025 года"),
            Promotion("Бесплатная доставка", "От 2000 ₽")
        )

        promotionsViewPager.adapter = PromotionsAdapter(promotions) {}
    }
    ...
    private fun setupSections() {
        ...
        sectionsRecycler.layoutManager = LinearLayoutManager(this)
    ...
    private fun getSampleBooks(): List<Book> =
        listOf(
            Book(1, "1984", listOf("Оруэлл"), "A", 350.0, rating = 4.5f),
            Book(2, "Мастер и Маргарита", listOf("Булгаков"), "B", 500.0, rating = 4.8f)
        )
}
